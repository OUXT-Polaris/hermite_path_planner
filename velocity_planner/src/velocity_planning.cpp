...
std::vector<size_t> getAdjacentIndex(const std::vector<VelocityConstraint>& constraints, size_t index)
{
...
double getSatisfiedVelocity(const double acceleration_limit, const VelocityConstraint& from, const VelocityConstraint& to){
    ...
    std::cout << "to.t:" << to.t << "," << "from.t:" << from.t << std::endl;
    if (acceleration > 0) {
        ...
        std::cout << "return(a>0):" <<std::sqrt(from.v * from.v + 2 * std::min(std::abs(acceleration), std::abs(acceleration_limit)) * (to.t - from.t)) << std::endl;
        return std::sqrt(from.v * from.v + 2 * std::min(std::abs(acceleration), std::abs(acceleration_limit)) * (to.t - from.t));

    }else{
        //std::cout << std::sqrt(from.v_limit * from.v_limit - 2 * std::min(std::abs(acceleration), std::abs(acceleration_limit)) * (to.t - from.t)) << std::endl;
        std::cout << "return(a<0):" << std::sqrt(to.v * to.v - 2 * std::min(std::abs(acceleration), std::abs(acceleration_limit)) * (from.t - to.t)) << std::endl;
        return std::sqrt(to.v * to.v - 2 * std::min(std::abs(acceleration), std::abs(acceleration_limit)) * (from.t - to.t));
    }
}

void updateAdjacentVelocity(std::vector<VelocityConstraint>& constraints, const double acceleration_limit, size_t target_index){
    ...
    for (const auto index:getAdjacentIndex(constraints, target_index)){
        if (!constraints[index].isChecked()) {
            ...
            constraints[index].v = getSatisfiedVelocity(acceleration_limit,
                constraints[std::min(index, target_index)], 
