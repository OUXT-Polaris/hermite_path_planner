...
double getSatisfiedVelocity(
  const double maximum_acceleration_limit, const double minimum_acceleration_limit, const VelocityConstraint & from, const VelocityConstraint & to)
{
  double acceleration_limit;
  if (from.v < to.v) {
    acceleration_limit = maximum_acceleration_limit;
  } else {
    acceleration_limit = minimum_acceleration_limit;
  }
...
void updateAdjacentVelocity(
  std::vector<VelocityConstraint> & constraints, const double maximum_acceleration_limit, const double minimum_acceleration_limit,
  size_t target_index)
{
  ...
  for (const auto index : getAdjacentIndex(constraints, target_index)) {
    if (!constraints[index].isChecked()) {
      constraints[index].v = getSatisfiedVelocity(
        maximum_acceleration_limit, minimum_acceleration_limit, constraints[std::min(index, target_index)],
...
std::vector<VelocityConstraint> planVelocity(
  std::vector<VelocityConstraint> constraints, double maximum_acceleration_limit, double minimum_acceleration_limit, double velocity_limit)
{
  clampVelocityConstraint(constraints, velocity_limit);
  updateAdjacentVelocity(constraints, maximum_acceleration_limit, minimum_acceleration_limit, constraints.size() - 1);
  while (!isAllConstraintsIsChecked(constraints)) {
    updateAdjacentVelocity(constraints, maximum_acceleration_limit, minimum_acceleration_limit, getMinimumIndex(constraints));
  }
  return constraints;
}
...
std::vector<hermite_path_msgs::msg::ReferenceVelocity> planVelocity(
  const std::vector<hermite_path_msgs::msg::ReferenceVelocity> & constraints,
  double maximum_acceleration_limit, double minimum_acceleration_limit, double velocity_limit)
{
  ...
  return toRos(planVelocity(fromRos(filtered_constraints), maximum_acceleration_limit, minimum_acceleration_limit, velocity_limit));
}
}  // namespace velocity_planning